...
const uint32_t kFlashSectorAddressMask = ~(kFlashSectorSize - 1); /*4096-1=0x00000fff取反 0xfffff000*/

const uint32_t kFlashMinAddress = 0x0001000;               /*4096首扇区保存配置信息 21.10.2把0x0001000改成0x0010000*/
const uint32_t kFlashMaxAddress = 0x1ffffff;   /*4096*8192=33554432*/
/* const uint32_t kFlashMaxAddress = 0x3fff; */
...
static const uint32_t rxBufferSize = 256+5;
...
NRF_QUEUE_DEF(flash_op_t, m_op_queue, 30, NRF_QUEUE_MODE_NO_OVERFLOW);
...
static uint8_t m_rx_buffer[rxBufferSize];
...
static void check_write(const uint8_t* p_data, uint16_t length, void* p_arguments);
...
void flash_init(void) {
  ...
  flash_op_t op1 = {
	...
	NRF_LOG_INFO("1 op: read queue: %d m_current_op.op_type=%d \n", op_queue_utilization_get(),m_current_op.op_type);
  err_code = op_queue_push(&op);
  NRF_LOG_INFO("2 op: read queue: %d m_current_op.op_type=%d \n", op_queue_utilization_get(),m_current_op.op_type);
	op_queue_pop((flash_op_t*)&m_current_op);
	NRF_LOG_INFO("3 op: read queue: %d m_current_op.op_type=%d \n", op_queue_utilization_get(),m_current_op.op_type);*/
}
...
void flash_test(void) {
  flash_enable();
  /* check_id(); */
  flash_erase(0, FLASH_ERASE_CHIP);
  flash_read(0, sizeof(kTestData), check_erase, NULL);
  //flash_write(kTestAddress, kTestData, sizeof(kTestData));
  //flash_read(kTestAddress, sizeof(kTestData), check_write, NULL);
...
void flash_read(uint32_t address, uint16_t length,
                flash_data_handler_t handler, void* p_arguments) {
	//21.10.3NRF_LOG_INFO("Enter FLASH READ0!\n");
  ...
  p_read_op->p_arguments = p_arguments;
  flash_op_t op = {
    .op_type = FLASH_READ,
    .p_actual_op = (void*)p_read_op
  };
  ret_code_t err_code = op_queue_push(&op); /*SPI*/
	//21.10.3NRF_LOG_INFO("(235)op_queue_push queue: %d  m_op_queue->op_type=%d \n", op_queue_utilization_get(),m_current_op.op_type);
  if (err_code == NRF_SUCCESS) {
		//21.10.3为了处理不跑read的问题
		if(op_queue_utilization_get()==2 &&m_current_op.op_type==FLASH_DISABLE){
      /*21.10.3ret_code_t err_code = op_queue_pop((flash_op_t*)&m_current_op);
      if(err_code==NRF_SUCCESS){
				NRF_LOG_INFO("(241)op_queue_push queue: %d  m_op_queue->op_type=%d \n", op_queue_utilization_get(),m_current_op.op_type);
				flash_op_t op1 = {
				.op_type = FLASH_NONE,
				.p_actual_op = NULL
				};
				err_code = op_queue_push(&op);
				if(err_code==NRF_SUCCESS){
					NRF_LOG_INFO("(248)op_queue_push queue: %d  m_op_queue->op_type=%d \n", op_queue_utilization_get(),m_current_op.op_type);
					op_start();
				}
      }*/
			m_current_op.op_type = FLASH_NONE;
			//21.10.3NRF_LOG_INFO("(253)op_queue_push queue: %d  m_op_queue->op_type=%d \n", op_queue_utilization_get(),m_current_op.op_type);
			op_start();
		}
		else{
			//21.10.3NRF_LOG_INFO("(257)op_queue_push queue: %d  m_op_queue->op_type=%d \n", op_queue_utilization_get(),m_current_op.op_type);
			op_start();
		}
	...
	//NRF_LOG_INFO("Enter FLASH READ4!\n");	
}
...
void flash_write(uint32_t address, const uint8_t* p_data, uint16_t length) {
  ...
  if ((address & ~kFlashPageAddressMask) + length > FLASH_PAGE_SIZE) {
    NRF_LOG_WARNING("Write over page, addr: %x p_data: %x len: %d\n",
  ...
  //NRF_LOG_INFO("flash write addr:%x len:%d\n", address, length);
...
void flash_enable(void) {
  ...
  if (err_code == NRF_SUCCESS) {
    op_start();
		//NRF_LOG_INFO("flash enable opstart finish!\n");
  } else if (err_code == NRF_ERROR_NO_MEM) {
    NRF_LOG_ERROR("Queue no memory!\n");
  } else {
    NRF_LOG_ERROR("Queue error: %d!\n", err_code);
...
void flash_disable(void) {
  ...
  if (err_code == NRF_SUCCESS) {
    op_start();
		//NRF_LOG_INFO("flash disable opstart finish!\n")
  } else if (err_code == NRF_ERROR_NO_MEM) {
    NRF_LOG_ERROR("Queue no memory!\n");
  } else {
    NRF_LOG_ERROR("Queue error: %d!\n", err_code);
...
//static uint8_t enter_op_counter=0;

static void op_start(void) {
  /* CRITICAL_REGION_ENTER(); */
	//NRF_LOG_INFO("op_start\n");
  if (m_current_op.op_type != FLASH_NONE || op_queue_is_empty()) {
		//NRF_LOG_INFO("m_current_op.op_type = %d  op_queue_is_empty()=%d\n",m_current_op.op_type,op_queue_is_empty());
		return;
  }
	//NRF_LOG_INFO("OUT m_current_op.op_type = %d  op_queue_is_empty()=%d\n",m_current_op.op_type,op_queue_is_empty());
  ...
  case FLASH_READ:
		//NRF_LOG_INFO("FLASH_READ\n");
    NRF_LOG_INFO("op: read queue: %d\n", op_queue_utilization_get());
    break;
  case FLASH_WRITE:
		//NRF_LOG_INFO("FLASH_WRITE\n");
    NRF_LOG_INFO("op: write queue: %d\n", op_queue_utilization_get());
    break;
  case FLASH_ERASE:
		//NRF_LOG_INFO("FLASH_ERASE\n");
    NRF_LOG_INFO("op: erase queue: %d\n", op_queue_utilization_get());
    break;
  case FLASH_ENABLE:
		//NRF_LOG_INFO("FLASH_ENABLE\n");
    NRF_LOG_INFO("op: enable queue: %d\n", op_queue_utilization_get());
    break;
  case FLASH_DISABLE:
		//NRF_LOG_INFO("FLASH_DISABLE\n");
    NRF_LOG_INFO("op: disable queue: %d\n", op_queue_utilization_get());
    break;
	default:
		NRF_LOG_INFO("FLASH NONE");
		break;
  }*/
  /* uint32_t address; */
  m_handler = op_end;
  ...
  default:
    NRF_LOG_INFO("Invalid operation type!\n");
    break;
  }
}
...
static void spi_handler(nrf_drv_spi_evt_t const* p_event) {
	//NRF_LOG_INFO("SPI handler success!\n");
...
static void op_end(void) {
  ...
  case FLASH_READ:
    ...
    if (handler) {
      handler(m_rx_buffer + 5,
              ((read_op_t*)m_current_op.p_actual_op)->length,
              ((read_op_t*)m_current_op.p_actual_op)->p_arguments);
    }
...
bool is_op_eq_four(void){
	return m_current_op.op_type == FLASH_NONE;
}