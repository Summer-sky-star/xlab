...
// kMeasurementDuration[repeatability_t]，最大测量时间，单位：毫秒
static const uint8_t kMeasurementDuration[3] = {4, 6, 15};
...
static const uint8_t kResetPin = 25;
...
void sht3_read_data(sht3_data_handler_t handler) {
  ...
  static app_twi_transfer_t command_transfer =
    APP_TWI_WRITE(SHT3_ADDRESS,
                  kSingleShotModeCommand[SHT3_HIGH_REPEATABLE][SHT3_CLK_STRETCH_DISABLED],
                  2,0);
  ...
  rtc_run_delay(kMeasurementDuration[SHT3_HIGH_REPEATABLE],
                kSHT3Channel, read_data, NULL);
  /* nrf_delay_ms(kMeasurementDuration[rep]); */
